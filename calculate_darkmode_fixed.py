...
# 💾 ฟังก์ชันจัดการฐานข้อมูล SQLite สำหรับระบบ Login และบันทึกต้นทุน
def init_db():
        ...
        if choice == "เข้าสู่ระบบ (Login)":
            ...
            if st.button("Sign In"):
                ...
                if user_data and check_hashes(password, user_data[0]):
                    ...
                    st.session_state.username = username # บันทึกชื่อผู้ใช้ใน Session เพื่อเอาไปใช้กับระบบต้นทุน
...
# ==========================================
    ...
    # 📊 สร้างแท็บเมนู 3 แท็บ
    tab1, tab2, tab3 = st.tabs(["🔮 พยากรณ์ผลผลิตด้วย AI", "🏗️ คำนวณโครงสร้างโรงเรือน", "📒 ระบบจดบันทึกต้นทุน"])
    ...
    with tab2:
        ...
        with calc_col1:
            ...
            # เพิ่มตัวเลือกหน่วยวัด (เมตร หรือ วา)
            unit_choice = st.selectbox("📏 เลือกหน่วยวัดที่ต้องการใช้งาน", ["เมตร (Meter)", "วา (Wa)"])
            unit_label = "เมตร" if unit_choice == "เมตร (Meter)" else "วา"
            area_unit = "ตารางเมตร" if unit_choice == "เมตร (Meter)" else "ตารางวา"
            
            width = st.number_input(f"📐 ความกว้างของโรงเรือน ({unit_label})", min_value=1.0, max_value=100.0, value=6.0, step=0.5)
            length = st.number_input(f"📐 ความยาวของโรงเรือน ({unit_label})", min_value=1.0, max_value=200.0, value=12.0, step=1.0)
            
            # กำหนดราคาตามเงื่อนไขหน่วย (ถ้า 'วา' ราคาต่อหน่วยจะคูณ 4 ตามสัดส่วนพื้นที่)
            if unit_choice == "เมตร (Meter)":
                p_std, p_prem, p_smart = 70, 100, 200
            else:
                p_std, p_prem, p_smart = 62.5 * 4, 100 * 4, 200 * 4
                
            material_prices = {
                "Standard (โครงเหล็กประปาทั่วไป + มุ้งขาวกันแมลงรอบทิศ)": p_std,
                "Premium (โครงกัลวาไนซ์กันสนิมอย่างดี + หลังคาพลาสติกกัน UV หนาพิเศษ)": p_prem,
                "Smart Greenhouse (ระบบโครงสร้างพรีเมียม + ชุดพ่นหมอกและเซนเซอร์อัตโนมัติ)": p_smart
            }
        ...
        with calc_col2:
            ...
            area = width * length
            price_per_unit = material_prices[material_choice]
            total_greenhouse_cost = area * price_per_unit
            
            # แปลงความยาวกลับเป็นเมตรในกรณีที่เลือกหน่วยเป็น 'วา' เพื่อคำนวณจำนวนเสาให้คงเดิม (1 วา = 2 เมตร)
            length_in_meters = length if unit_choice == "เมตร (Meter)" else length * 2
            estimated_poles = int((length_in_meters / 2) * 2 + 2)
            ...
            st.markdown(f"""
                <div class='result-card' style='background: linear-gradient(135deg, #2c3e50, #3498db);'>
                    ...
                    <p style='font-size: 34px; font-weight: bold; margin: 10px 0; color: #ecf0f1;'>{area:.2f} <span style='font-size: 18px;'>{area_unit}</span></p>
                </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(f"""
                <div class='result-card' style='background: linear-gradient(135deg, #27ae60, #2ecc71);'>
                    ...
                    <p style='font-size: 34px; font-weight: bold; margin: 10px 0; color: #fff;'>{total_greenhouse_cost:,.2f} <span style='font-size: 18px;'>บาท</span></p>
                    <small style='color: #eaeded;'>*คำนวณจากราคาวัสดุเริ่มต้นเฉลี่ยประมาณ {price_per_unit} บาท / {area_unit}</small>
                </div>
            """, unsafe_allow_html=True)
            ...
            item_df = pd.DataFrame({
                "รายการพัสดุ": [f"พื้นที่ผืนพลาสติกคลุมหลังคา / มุ้งลวด", "จำนวนเสาโครงสร้างหลัก (ระยะติดตั้งทุก 2 เมตร)", "ราคาประเมินวัสดุอุปกรณ์รวมขั้นต่ำ"],
                "ปริมาณโดยประมาณ": [f"{area:.2f} {area_unit}", f"อย่างน้อย {estimated_poles} ต้น", f"{total_greenhouse_cost:,.2f} บาท"]
            })
            st.dataframe(item_df, use_container_width=True, hide_index=True)

    # ---------------- TAB 3: ระบบจดบันทึกต้นทุน (เพิ่มใหม่) ----------------
    with tab3:
        cost_col1, cost_col2 = st.columns([4, 6], gap="large")
        
        with cost_col1:
            st.markdown("<h3 style='color: #11998e; margin-bottom: 15px;'>📝 ระบุข้อมูลต้นทุน (บาท)</h3>", unsafe_allow_html=True)
            
            water_cost = st.number_input("💧 ค่าน้ำ", min_value=0.0, value=0.0, step=50.0)
            electric_cost = st.number_input("⚡ ค่าไฟฟ้า", min_value=0.0, value=0.0, step=50.0)
            fertilizer_cost = st.number_input("🧪 ค่าปุ๋ย", min_value=0.0, value=0.0, step=50.0)
            
            if st.button("💾 บันทึกต้นทุน", use_container_width=True):
                total_cost = water_cost + electric_cost + fertilizer_cost
                ...
                try:
                    ...
                    c = conn.cursor()
                    c.execute("""
                        INSERT INTO costs (username, water_cost, electric_cost, fertilizer_cost, total_cost) 
                        VALUES (?, ?, ?, ?, ?)
                    """, (current_user, water_cost, electric_cost, fertilizer_cost, total_cost))
                    conn.commit()
                    conn.close()
                    
                    st.success("✅ บันทึกข้อมูลต้นทุนเรียบร้อยแล้ว!")
                except Exception as e:
                    st.error(f"❌ เกิดข้อผิดพลาดในการบันทึกข้อมูล: {e}")
        
        with cost_col2:
            st.markdown("<h3 style='color: #11998e; margin-bottom: 15px;'>📊 ประวัติการบันทึกต้นทุนของคุณ</h3>", unsafe_allow_html=True)
            
            current_user = st.session_state.username
            conn = sqlite3.connect("users.db")
            
            # ดึงข้อมูลจากฐานข้อมูลมาแสดงใน DataFrame
            query = f"""
                SELECT 
                    record_date AS 'วันและเวลาที่บันทึก', 
                    water_cost AS 'ค่าน้ำ (บาท)', 
                    electric_cost AS 'ค่าไฟ (บาท)', 
                    fertilizer_cost AS 'ค่าปุ๋ย (บาท)', 
                    total_cost AS 'รวมต้นทุน (บาท)' 
                FROM costs 
                WHERE username = '{current_user}' 
                ORDER BY id DESC
            """
            df_costs = pd.read_sql_query(query, conn)
            conn.close()
            
            if not df_costs.empty:
                st.dataframe(df_costs, use_container_width=True, hide_index=True)
                
                # สรุปผลรวมต้นทุนทั้งหมดที่เคยบันทึก
                total_all_time = df_costs['รวมต้นทุน (บาท)'].sum()
                st.markdown(f"**รวมค่าใช้จ่ายทั้งหมดที่คุณเคยบันทึก:** `{total_all_time:,.2f}` บาท")
            else:
                st.info("📌 ยังไม่มีประวัติการบันทึกต้นทุนในระบบ เริ่มต้นจดบันทึกทางด้านซ้ายได้เลย")